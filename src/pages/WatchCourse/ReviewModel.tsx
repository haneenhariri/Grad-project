...
export default function ReviewModel({
  ...
  const handleSubmit = async () => {
    ...
    if (!courseId) {
      ...
      showToast("Cannot submit review: Course ID is missing", "error");
      return;
    }

    try {
      setIsSubmitting(true);
      console.log(`Submitting review - Course ID: ${courseId}, Rating: ${rating}, Review: ${review}`);
      ...
      if (axios.isAxiosError(error)) {
        const errorMessage = error.response?.data?.message || error.message || "Failed to submit your review";
  ...
  return (
    showMode && (   
    <div className="fixed inset-0 bg-black bg-opacity-50 md:text-lg text-sm flex items-center justify-center z-50">
        <div className="bg-white rounded-lg w-full max-w-lg shadow-lg">
            <div className="p-5 flex justify-between border-b items-center">
                ...
                <button onClick={() => setShowMode(false)} className="text-gray-500 hover:text-gray-700">
            ...
            <div className="p-5 flex justify-between items-center border-t">
                <Button text={t("Cancel")} onClick={() => setShowMode(false)} Bg="bg-gray-400/15" /> 
                <Button 
                  text={isSubmitting ? t("Submitting...") : t("Submit Review")} 
                  Bg={rating > 0 ? "bg-btn text-white" : "bg-gray-300 text-gray-600"} 
