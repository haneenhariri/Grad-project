...
export default function SignUp({btn} : logbtn) {
  ...
  return (
    <form className="md:mb-6 " onSubmit={send}>
      <label className='mb-2.5 font-medium  md:text-base text-sm block' htmlFor="name">{t("Name")}</label>
      <input
        className="w-full mb-5 lg:p-5 p-2.5 placeholder: md:text-base text-sm bg-White/95 rounded-md"
        placeholder={t("NamePlace")}
        type="text"
        defaultValue={formData.name}
        onChange={(e) => setFormData( {...formData, name: e.target.value})}
      />
      <label className='mb-2.5 font-medium  md:text-base text-sm block' htmlFor="email">{t("Email")}</label>
      <input
        className="w-full mb-5 lg:p-5 p-2.5 placeholder: md:text-base text-sm bg-White/95 rounded-md"
      ...
      <div>
      <label className='mb-2.5 font-medium  md:text-base text-sm block' htmlFor="">{t("Password")}</label>
      ...
      <button className="w-full text-white lg:py-4.5 lg:px-5 p-2.5 md:text-base text-sm md:mb-0 mb-5 rounded-lg bg-violet-950">{t(btn)}</button>
    </form>
  )
} 
