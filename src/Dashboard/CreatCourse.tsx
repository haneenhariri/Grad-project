...
export default function CreatCourse() {
    ...
    const handleSubmit = (e: React.FormEvent) => {
      ...
      mutation.mutate(formData, {
        onSuccess: (data) => {
          setCourseId(data.id); // تخزين معرف الكورس
...
/>      <div className='mt-5 flex justify-between'>
          <Button text='Cancel' textColor='border-gray border text-violet-950 ' />
          <Button text='Save Course' type='submit' textColor=' text-white ' onClick={() => handleSubmit} Bg='bg-violet-950' />
        </div>
      </div>
